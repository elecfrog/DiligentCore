...
public:
    ...
    void  Begin(UInt32 ImmediateContextId) override final;
    ...
    void  TransitionShaderResources(IShaderResourceBinding* pShaderResourceBinding) override final;
    ...
    void  SetStencilRef(UInt32 StencilRef) override final;

    /// Implementation of IDeviceContext::SetBlendFactors() in WebGPU backend.
    void  SetBlendFactors(const float* pBlendFactors = nullptr) override final;
    ...
    void  InvalidateState() override final;
    ...
    void  Draw(const DrawAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::DrawIndexed() in WebGPU backend.
    void  DrawIndexed(const DrawIndexedAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::DrawIndirect() in WebGPU backend.
    void  DrawIndirect(const DrawIndirectAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::DrawIndexedIndirect() in WebGPU backend.
    void  DrawIndexedIndirect(const DrawIndexedIndirectAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::DrawMesh() in WebGPU backend.
    void  DrawMesh(const DrawMeshAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::DrawMeshIndirect() in WebGPU backend.
    void  DrawMeshIndirect(const DrawMeshIndirectAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::MultiDraw() in WebGPU backend.
    void  MultiDraw(const MultiDrawAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::MultiDrawIndexed() in WebGPU backend.
    void  MultiDrawIndexed(const MultiDrawIndexedAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::DispatchCompute() in WebGPU backend.
    void  DispatchCompute(const DispatchComputeAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::DispatchComputeIndirect() in WebGPU backend.
    void  DispatchComputeIndirect(const DispatchComputeIndirectAttribs& Attribs) override final;
    ...
    void  UnmapBuffer(IBuffer* pBuffer, MAP_TYPE MapType) override final;
    ...
    void  CopyTexture(const CopyTextureAttribs& CopyAttribs) override final;
    ...
    void  BuildBLAS(const BuildBLASAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::BuildTLAS() in WebGPU backend.
    void  BuildTLAS(const BuildTLASAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::CopyBLAS() in WebGPU backend.
    void  CopyBLAS(const CopyBLASAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::CopyTLAS() in WebGPU backend.
    void  CopyTLAS(const CopyTLASAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::WriteBLASCompactedSize() in WebGPU backend.
    void  WriteBLASCompactedSize(const WriteBLASCompactedSizeAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::WriteTLASCompactedSize() in WebGPU backend.
    void  WriteTLASCompactedSize(const WriteTLASCompactedSizeAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::TraceRays() in WebGPU backend.
    void  TraceRays(const TraceRaysAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::TraceRaysIndirect() in WebGPU backend.
    void  TraceRaysIndirect(const TraceRaysIndirectAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::UpdateSBT() in WebGPU backend.
    void  UpdateSBT(IShaderBindingTable* pSBT, const UpdateIndirectRTBufferAttribs* pUpdateIndirectBufferAttribs) override final;

    /// Implementation of IDeviceContext::BeginDebugGroup() in WebGPU backend.
    void  BeginDebugGroup(const Char* Name, const float* pColor) override final;
    ...
    void  InsertDebugLabel(const Char* Label, const float* pColor) override final;

    /// Implementation of IDeviceContext::SetShadingRate() in WebGPU backend.
    void  SetShadingRate(SHADING_RATE          BaseRate,
    ...
    void  BindSparseResourceMemory(const BindSparseResourceMemoryAttribs& Attribs) override final;

    /// Implementation of IDeviceContext::GenerateMips() in WebGPU backend.
    void  GenerateMips(ITextureView* pTexView) override final;

    /// Implementation of IDeviceContext::FinishFrame() in WebGPU backend.
    void  FinishFrame() override final;

    /// Implementation of IDeviceContext::TransitionResourceStates() in WebGPU backend.
    void  TransitionResourceStates(UInt32 BarrierCount, const StateTransitionDesc* pResourceBarriers) override final;

    /// Implementation of IDeviceContext::LockCommandQueue() in WebGPU backend.
    ICommandQueue*  LockCommandQueue() override final;

    /// Implementation of IDeviceContext::UnlockCommandQueue() in WebGPU backend.
    void  UnlockCommandQueue() override final;

    /// Implementation of IDeviceContext::ResolveTextureSubresource() in WebGPU backend.
    void  ResolveTextureSubresource(ITexture*                               pSrcTexture,
    ...
    WGPUQueue  GetWebGPUQueue() override final;
