...
public:
    ...
    void  Present(UInt32 SyncInterval) override final;

    /// Implementation of ISwapChain::Resize() in Direct3D11 backend.
    void  Resize(UInt32 NewWidth, UInt32 NewHeight, SURFACE_TRANSFORM NewPreTransform) override final;

    /// Implementation of ISwapChain::SetFullscreenMode() in WebGPU backend.
    void  SetFullscreenMode(const DisplayModeAttribs& DisplayMode) override final;

    /// Implementation of ISwapChain::SetWindowedMode() in WebGPU backend.
    void  SetWindowedMode() override final;

    /// Implementation of ISwapChainWebGPU::GetCurrentBackBufferRTV() in WebGPU backend.
    ITextureViewWebGPU*  GetCurrentBackBufferRTV() override final { return m_pBackBufferRTV; }

    /// Implementation of ISwapChainWebGPU::GetDepthBufferDSV() in WebGPU backend.
    ITextureViewWebGPU*  GetDepthBufferDSV() override final { return m_pDepthBufferDSV; }

    /// Implementation of ISwapChainWebGPU::GetWebGPUSurface() in WebGPU backend.
    WGPUSurface  GetWebGPUSurface() override final { return m_wgpuSurface.Get(); }
