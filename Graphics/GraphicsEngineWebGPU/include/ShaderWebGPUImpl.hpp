...
public:
    ...
    UInt32  GetResourceCount() const override final;

    /// Implementation of IShader::GetResourceDesc() in WebGPU backend.
    void  GetResourceDesc(UInt32 Index, ShaderResourceDesc& ResourceDesc) const override final;

    /// Implementation of IShader::GetConstantBufferDesc() in WebGPU backend.
    const ShaderCodeBufferDesc*  GetConstantBufferDesc(UInt32 Index) const override final;

    /// Implementation of IShader::GetBytecode() in WebGPU backend.
    void  GetBytecode(const void** ppBytecode, UInt64& Size) const override final;

    /// Implementation of IShaderWebGPU::GetWGSL().
    const std::string&  GetWGSL() const override final;

    /// Implementation of IShaderWebGPU::GetEmulatedArrayIndexSuffix().
    const char*  GetEmulatedArrayIndexSuffix() const override final
    {
