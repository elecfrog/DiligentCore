...
public:
    ...
    virtual UInt64 DG_CALL_TYPE Submit(UInt32                    NumCommandLists,
                                             ID3D12CommandList* const* ppCommandLists) override final;

    // Implementation of ICommandQueueD3D12::GetD3D12CommandQueue().
    virtual ID3D12CommandQueue* DG_CALL_TYPE GetD3D12CommandQueue() override final { return m_pd3d12CmdQueue; }

    // Implementation of ICommandQueueD3D12::EnqueueSignal().
    virtual void DG_CALL_TYPE EnqueueSignal(ID3D12Fence* pFence, UInt64 Value) override final;

    // Implementation of ICommandQueueD3D12::WaitFence().
    virtual void DG_CALL_TYPE WaitFence(ID3D12Fence* pFence, UInt64 Value) override final;

    // Implementation of ICommandQueueD3D12::GetD3D12CommandQueueDesc().
    virtual const D3D12_COMMAND_QUEUE_DESC& DG_CALL_TYPE GetD3D12CommandQueueDesc( ) const override final
    {
        return m_d3d12CmdQueueDesc;
    }

    // Implementation of ICommandQueueD3D12::UpdateTileMappings().
    virtual void DG_CALL_TYPE UpdateTileMappings(ResourceTileMappingsD3D12* pMappings,
