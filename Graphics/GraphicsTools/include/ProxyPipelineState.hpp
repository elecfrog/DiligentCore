...
public:
    ...
    virtual IObject*  GetUserData() const override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
        return m_pPipeline ? m_pPipeline->GetUserData() : nullptr;
    }

    virtual const GraphicsPipelineDesc&  GetGraphicsPipelineDesc() const override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
    ...
    virtual const RayTracingPipelineDesc&  GetRayTracingPipelineDesc() const override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
    ...
    virtual const TilePipelineDesc&  GetTilePipelineDesc() const override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
    ...
    virtual void  BindStaticResources(SHADER_TYPE ShaderStages, IResourceMapping* pResourceMapping, BIND_SHADER_RESOURCES_FLAGS Flags) override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
        if (m_pPipeline)
        {
    ...
    virtual UInt32  GetStaticVariableCount(SHADER_TYPE ShaderType) const override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
        return m_pPipeline ? m_pPipeline->GetStaticVariableCount(ShaderType) : 0;
    }

    virtual IShaderResourceVariable*  GetStaticVariableByName(SHADER_TYPE ShaderType, const Char* Name) override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
        return m_pPipeline ? m_pPipeline->GetStaticVariableByName(ShaderType, Name) : nullptr;
    }

    virtual IShaderResourceVariable*  GetStaticVariableByIndex(SHADER_TYPE ShaderType, UInt32 Index) override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
        return m_pPipeline ? m_pPipeline->GetStaticVariableByIndex(ShaderType, Index) : nullptr;
    }

    virtual void  CreateShaderResourceBinding(IShaderResourceBinding** ppShaderResourceBinding, bool InitStaticResources) override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
        if (m_pPipeline)
        {
    ...
    virtual void  InitializeStaticSRBResources(IShaderResourceBinding* pShaderResourceBinding) const override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
        if (m_pPipeline)
        {
    ...
    virtual void  CopyStaticResources(IPipelineState* pPSO) const override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
        if (m_pPipeline)
        {
    ...
    virtual UInt32  GetResourceSignatureCount() const override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
        return m_pPipeline ? m_pPipeline->GetResourceSignatureCount() : 0;
    }

    virtual IPipelineResourceSignature*  GetResourceSignature(UInt32 Index) const override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
        return m_pPipeline ? m_pPipeline->GetResourceSignature(Index) : nullptr;
    }

    virtual PIPELINE_STATE_STATUS  GetStatus(bool WaitForCompletion) override
    {
        DEV_CHECK_ERR(m_pPipeline, "Internal pipeline is null");
